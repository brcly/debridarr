...
test('Prowlarr search sends query and categories, keeps torrents, and normalizes fields', async t => {
  let requestUrl = '';
  const base = await listen(createServer((request, response) => {
  ...
  assert.deepEqual(releases.map(r => r.title), ['The Matrix 1999 1080p', 'The Matrix 1999 720p']);
  assert.equal(releases[0]!.infoHash, 'abcdef0123456789abcdef0123456789abcdef01');
  assert.equal(releases[0]!.magnetUrl, 'magnet:?x');
  assert.equal(releases[1]!.seeders, 0, 'negative seeders clamp to 0');
  assert.equal(releases[1]!.size, 0, 'non-numeric size clamps to 0');
});
