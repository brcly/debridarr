...
test('one login is reused across calls; add/files/priorities/shareLimits/delete hit the right endpoints', async t => {
  ...
  assert.equal((await qbt.torrent(HASH, signal))?.name, 'The Matrix 1999 1080p');
  assert.equal((await qbt.torrent('f'.repeat(40), signal)), undefined);
  await qbt.add('magnet:?xt=urn:btih:' + HASH, { category: 'debridarr' }, signal);
  const files = await qbt.files(HASH, signal);
  assert.deepEqual(files.map(f => f.index), [0, 1]);
  await qbt.setFilePriorities(HASH, [1], 0, signal);
  await qbt.setShareLimits(HASH, { ratioLimit: 1 }, signal);
  await qbt.delete(HASH, true, signal);
  const byCategory = await qbt.torrentsByCategory('debridarr', signal);
  ...
  assert.deepEqual(byCategory.map(t => t.hash), [HASH]);
...
test('an expired session (403) triggers exactly one re-login and retry', async t => {
  ...
  const qbt = new QBittorrentClient({ url: base, username: 'admin', password: 'pw' });
  const torrent = await qbt.torrent(HASH, AbortSignal.timeout(3000));
...
test('addTorrentFile uploads the raw bytes as multipart form data', async t => {
  ...
  await qbt.addTorrentFile(bytes, { category: 'debridarr' }, AbortSignal.timeout(3000));
