...
import { DEBRIDARR_SCOPE, DownloadError, ensureTransfer, type EnsureOptions } from '../src/downloads/manager.js';
...
async function store(t: TestContext) {
  const dir = await tmpDir(t, 'debridarr-mgr');
  return DownloadsStore.open(dir);
}

const movie: PlayShape = { title: 'The Matrix 1999 1080p BluRay', size: 100, imdbId: 'tt0133093', type: 'movie', infoHash: HASH };
...
test('adds a missing torrent, enables sequential + first/last, sets the ratio limit, and records it', async t => {
  ...
  const state = await ensureDownload(movie, { backend: new QBittorrentClient({ url: base, username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(5000) });
  ...
  assert.equal(state.file.id, 0, 'largest non-sample video');
  assert.equal(state.record.infoHash, HASH);
  assert.equal(state.record.origin, 'search');
  assert.equal(state.record.media?.imdbId, 'tt0133093');
...
test('choosing a season pack keeps the whole season downloading, parking only non-episode files', async t => {
  const torrent = {
    present: false, appearAfter: 1, seq: false, flpp: false,
    files: [
      { index: 0, name: 'The.Show.S03.1080p.WEB-DL/The.Show.S03E01.mkv', size: 1_000_000_000, progress: 0, priority: 1 },
      { index: 1, name: 'The.Show.S03.1080p.WEB-DL/The.Show.S03E02.mkv', size: 1_000_000_000, progress: 0, priority: 1 },
      { index: 2, name: 'The.Show.S03.1080p.WEB-DL/The.Show.S03E03.mkv', size: 1_000_000_000, progress: 0, priority: 1 },
      { index: 3, name: 'The.Show.S03.1080p.WEB-DL/sample.mkv', size: 20_000_000, progress: 0, priority: 1 },
      { index: 4, name: 'The.Show.S03.1080p.WEB-DL/readme.nfo', size: 500, progress: 0, priority: 1 },
    ],
  };
  const { base } = await fakeQbt(t, torrent);
  const s = await store(t);
  const pack: PlayShape = { title: 'The Show S03 1080p WEB-DL', size: 3_000_000_000, imdbId: 'tt7', type: 'series', season: 3, episode: 2, infoHash: HASH };
  const state = await ensureDownload(pack, { backend: new QBittorrentClient({ url: base, username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(5000) });

  const selected = s.get(HASH)?.selectedFiles ?? [];
  assert.equal(state.file.path, 'The.Show.S03.1080p.WEB-DL/The.Show.S03E02.mkv', 'streams the requested episode');
  assert.deepEqual(selected.map(f => f.index).sort(), [0, 1, 2], 'every season-3 episode is selected');
  assert.equal(selected.find(f => f.index === 0)?.media?.episode, 1, 'siblings carry their own episode identity');
  assert.equal(selected.find(f => f.index === 1)?.auto, undefined, 'the played episode is an explicit selection');
  assert.equal(selected.find(f => f.index === 0)?.auto, true, 'the rest of the season is auto-enqueued');
  assert.deepEqual(torrent.files.map(f => f.priority), [1, 1, 1, 0, 0], 'episodes stay enabled; sample and nfo are parked');
});
...
test('refuses an unrelated existing torrent without adding, serving or reprioritizing', async t => {
  ...
  const s = await store(t);
  await assert.rejects(ensureDownload(movie, { backend: new QBittorrentClient({ url: base, username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(5000) }), /ownership/);
...
test('a target with no infohash, magnet, or download URL is rejected before any request', async t => {
  ...
  await assert.rejects(
    ensureDownload({ title: 'x', size: 1, imdbId: 'tt1', type: 'movie' }, { backend: new QBittorrentClient({ url: 'http://127.0.0.1:1', username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(2000) }),
...
test('a downloadUrl whose torrent file cannot be fetched fails with torrent_fetch_failed', async t => {
  ...
  await assert.rejects(
    ensureDownload(
      ...
      { backend: new QBittorrentClient({ url: 'http://127.0.0.1:1', username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(2000) },
    ),
...
test('a downloadUrl-only release fetches the .torrent file and uploads its bytes (preserving trackers), keyed by the computed infohash', async t => {
  ...
  const s = await store(t);
  const target: PlayShape = { title: 'The Matrix 1999 1080p BluRay', size: 100, imdbId: 'tt0133093', type: 'movie', downloadUrl: `${torrentUrl}/1/download?link=fixture` };
  const state = await ensureDownload(target, {
    sourceResolver: (url, signal) => fetchTorrentSource(url, signal, [{ url: torrentUrl, apiKey: 'test-key', type: 'prowlarr' }]),
    backend: new QBittorrentClient({ url: qbtBase, username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(5000),
  });
...
test('DEBRIDARR_SCOPE is the qBittorrent category used on add', async t => {
  ...
  const s = await store(t);
  await ensureDownload(movie, { backend: new QBittorrentClient({ url: base, username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(5000) });
  assert.equal(DEBRIDARR_SCOPE, 'debridarr');
  assert.ok(seen.includes('/api/v2/torrents/add'));
});
...
test('a freshly added stopped torrent is started before its files are read, so magnets can fetch metadata', async t => {
  ...
  const s = await store(t);
  const target: PlayShape = { title: 'The Lost Boys 1987 REMASTERED 1080p BluRay x265-RBG', size: 100, imdbId: 'tt0093437', type: 'movie', infoHash: HASH };
  const state = await ensureDownload(target, { backend: new QBittorrentClient({ url: base, username: 'u', password: 'p' }), store: s, signal: AbortSignal.timeout(5000) });
