...
function sameOrigin(candidate: string, url: URL): boolean {
  try { return new URL(candidate).origin === url.origin; } catch { return false; }
}
// A configured discovery provider's origin is the only thing trusted: a
// Prowlarr-type match gets the strict `/<n>/download` + `link` shape check
// (its download links have a fixed shape), stripping any embedded apikey so
// an old search result can never reuse one; any other provider type gets a
// looser non-root-path check and has its own apiKey filled in as the
// Newznab `apikey` parameter when the link carries none.
export function validateProxy(raw: string, providers: readonly ProxyTarget[] = []): URL {
  const url = new URL(raw);
  if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || url.hash) throw new Error('Invalid torrent proxy');
  const provider = providers.find(candidate => candidate.url && sameOrigin(candidate.url, url));
  if (!provider) throw new Error('Invalid torrent proxy');
  if (provider.type === 'prowlarr') {
    const base = new URL(provider.url);
    const prefix = base.pathname.replace(/\/$/, '');
    if (!url.pathname.startsWith(prefix + '/') || !/^\/\d+\/download$/.test(url.pathname.slice(prefix.length))
      || !url.searchParams.get('link')) throw new Error('Invalid torrent proxy');
    // Never reuse a key embedded in an old search result.
    url.searchParams.delete('apikey');
    return url;
  }
  if (url.pathname === '/' || url.pathname === '') throw new Error('Invalid torrent proxy');
  if (provider.apiKey && !url.searchParams.has('apikey')) url.searchParams.set('apikey', provider.apiKey);
  return url;
}
...
export type TorrentSource =
  | { bytes: Buffer; magnet?: never; nzb?: never }
  | { magnet: string; bytes?: never; nzb?: never }
  | { nzb: Buffer; bytes?: never; magnet?: never };
export async function fetchTorrentSource(raw: string, signal: AbortSignal, providers: readonly ProxyTarget[] = []): Promise<TorrentSource> {
  let url = validateProxy(raw, providers);
  for (let hop = 0; hop <= 5; hop++) {
    ...
    signal.throwIfAborted();
    // Only a Prowlarr-type provider authenticates by header; any other
    // provider's key travels as its own apikey parameter (added in
    // validateProxy) and must not be replaced.
    const apiKey = hop === 0
      ? providers.find(provider => provider.type === 'prowlarr' && provider.url && sameOrigin(provider.url, url))?.apiKey ?? ''
      : '';
    const result = await new Promise<{ location?: string; bytes?: Buffer }>((resolve, reject) => {
      const req = (url.protocol === 'https:' ? httpsRequest : httpRequest)(url, {
        signal, agent: false,
        headers: { Accept: 'application/x-bittorrent, application/x-nzb, application/xml;q=0.9, */*;q=0.8', ...(apiKey ? { 'X-Api-Key': apiKey } : {}) },
    ...
    if (result.bytes) return isNzb(result.bytes) ? { nzb: result.bytes } : { bytes: result.bytes };
