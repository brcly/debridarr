...
// The display name from a .torrent file's `info.name`, for records and Stremio
// labels. Same lenient bencode walk as parseInfoHash: undefined on anything
// malformed or without the key.
export function parseTorrentName(buffer: Buffer): string | undefined {
  try {
    const pos = { i: 0 };
    if (buffer[pos.i] !== 0x64 /* 'd' */) return undefined;
    pos.i += 1;
    for (;;) {
      if (pos.i >= buffer.length || buffer[pos.i] === 0x65 /* 'e' */) return undefined;
      const key = readStringSpan(buffer, pos);
      if (buffer.toString('latin1', key.start, key.end) !== 'info') { skipValue(buffer, pos, 0); continue; }
      if (buffer[pos.i] !== 0x64) return undefined;
      pos.i += 1;
      for (;;) {
        if (pos.i >= buffer.length || buffer[pos.i] === 0x65) return undefined;
        const infoKey = readStringSpan(buffer, pos);
        if (buffer.toString('latin1', infoKey.start, infoKey.end) === 'name') {
          const span = readStringSpan(buffer, pos);
          return buffer.toString('utf8', span.start, span.end).trim() || undefined;
        }
        skipValue(buffer, pos, 0);
      }
    }
